  ...
  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background:#f4f2f8;padding:24px 12px;">
    <tr><td align="center">
      <table role="presentation" width="100%" style="max-width:560px;background:#ffffff;border-radius:12px;overflow:hidden;border:1px solid #e8e4f0;">
        <tr><td style="background:${BRAND_BG};padding:20px 24px;color:${BRAND_TEXT};font-size:18px;font-weight:600;">True Goshen</td></tr>
        <tr><td style="padding:28px 24px;">${bodyHtml}</td></tr>
        <tr><td style="padding:16px 24px;background:#faf8fc;font-size:12px;color:#6b6478;">This message was sent by True Goshen. If you did not expect it, you can ignore this email.</td></tr>
      </table>
    </td></tr>
  </table>
</body></html>`;
...
export function welcomeEmail(
  ...
  const subject = "Welcome to True Goshen — your account is ready";
  const html = layout(
    subject,
    `<h1 style="margin:0 0 12px;font-size:22px;">Welcome${name ? `, ${escapeHtml(name)}` : ""}</h1><p>Your True Goshen account has been created. You are signed in and can track pre-orders, message our team, and manage your preferences anytime.</p>${welcomeTrackingHtml(details)}${cta(accountUrl, "Open my account")}`
  );
  const text = `Welcome to True Goshen${name ? `, ${name}` : ""}.
...
export function verifyEmail(name: string, verifyUrl: string): BrandedEmail {
  const subject = "Confirm your True Goshen email";
...
export function passwordResetEmail(name: string, resetUrl: string): BrandedEmail {
  ...
  return { subject, html, text: `Reset your True Goshen password: ${resetUrl}` };
}

export function inviteEmail(name: string, inviteUrl: string, roleLabel: string): BrandedEmail {
  const subject = "You are invited to True Goshen Platform";
  const safeName = name ? ` ${escapeHtml(name)}` : "";
  const html = layout(
    subject,
  ...
  return { subject, html, text: `You are invited to True Goshen Platform (${roleLabel}): ${inviteUrl}` };
}
...
export function passwordChangedEmail(
  ...
  const subject = "Your True Goshen password was changed";
  const safeName = name ? ` ${escapeHtml(name)}` : "";
  ...
  const textLines = [
    `Hi${name ? ` ${name}` : ""}, your True Goshen password was changed successfully.`,
    when ? `When: ${when}` : "",
...
export function loginAlertEmail(name: string, detail: { when: string; ip?: string; device?: string }): BrandedEmail {
  const subject = "New True Goshen sign-in";
  const safeName = name ? ` ${escapeHtml(name)}` : "";
  const html = layout(
    subject,
  ...
  return { subject, html, text: `New True Goshen sign-in at ${detail.when}. If this was not you, change your password.` };
}

export function newDeviceEmail(name: string, device: string): BrandedEmail {
  const subject = "New device signed in to True Goshen";
  const safeName = name ? ` ${escapeHtml(name)}` : "";
  const html = layout(
    subject,
...
export function mfaChangedEmail(name: string, enabled: boolean): BrandedEmail {
  const subject = enabled
    ? "Authenticator enabled on your True Goshen account"
    : "Authenticator removed from your True Goshen account";
  const safeName = name ? ` ${escapeHtml(name)}` : "";
  const html = layout(
    subject,
