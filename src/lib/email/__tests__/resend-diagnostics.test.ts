...
const FROM = "True Goshen <noreply@truegoshengh.com>";
...
describe("buildResendDiagnostics", () => {
  ...
  it("passes when the key can see the From domain as verified", () => {
    ...
    expect(report.fromAddress).toBe("noreply@truegoshengh.com");
    expect(report.fromDomain).toBe("truegoshengh.com");
  ...
  it("identifies a key scoped to a different Resend team", () => {
    ...
    expect(report.verdict).toContain("does not contain truegoshengh.com");
  ...
  it("matches the domain case-insensitively", () => {
    const report = buildResendDiagnostics({
      apiKey: API_KEY,
      fromEmail: "noreply@TrueGoshenGH.com",
      domainList: listed(verified({ name: "TRUEGOSHENGH.COM" })),
      environment: PRODUCTION,
    });
