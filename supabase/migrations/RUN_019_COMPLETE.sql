-- RUN_019_COMPLETE.sql
-- Idempotent recovery for a failed 019_customer_conversations migration.
-- Paste this entire file into the Supabase SQL Editor and run once (safe to re-run).
--
-- Use when:
--   • 019 rolled back and customer_conversations does not exist
--   • 021 fails with "relation customer_conversations does not exist"
--   • profiles.registration_id was missing during the original 019 run

-- 1. profiles.registration_id + sequence + generate_registration_id() FIRST
...
-- 2. customer_conversations + customer_conversation_messages
...
-- 3. Migrate legacy customer_messages if that table exists
DO $$
BEGIN
...
-- 4. Backfill registration_id on conversations from profiles
...
-- 5. RLS policies, trigger, realtime publication
